	...
	public void draw(Canvas canvas, float xPos, float yPos) {
		...
		if (wayPoint){
			...
			if (last){
				...
				canvas.drawLine(xPos+tileWidth/4, yPos+3*tileHeight/5, xPos+3*tileWidth/4, yPos+9*tileHeight/10, color);
				canvas.drawLine(xPos+tileWidth/4, yPos+9*tileHeight/10, xPos+3*tileWidth/4, yPos+3*tileHeight/5, color);
			}else
				canvas.drawCircle(xPos+tileWidth/2, yPos+3*tileHeight/4, tileWidth/10, color);
		}
	}
}
